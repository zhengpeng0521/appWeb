 ...
 *author 赵健
 */
export function zj_menu_format(mockMenu){
    let menuList = listToTree(objListSort(mockMenu, 'seq_no'), 0);
    let allMenuList = [];
    if(menuList && menuList.length > 0) {
        let middleArr = [];
        for(let i in menuList) {
            let { menu_key , name , seq_no , children } = menuList[i];
            middleArr.push({ menu_key , name , seq_no , children : listToGroup(children) });
        }
        for(let i in middleArr){
            for(let j in middleArr[i].children[0]){
                //去除无子元素的菜单(一般是首页)
                if(!!middleArr[i].children[0][j].children && middleArr[i].children[0][j].children.length > 0){
                    for(let k in middleArr[i].children[0][j].children){
                        middleArr[i].children[0][j].children[k].choose = false;
                    }
                    allMenuList.push(middleArr[i].children[0][j])
                }
            }
        }
        return allMenuList;
