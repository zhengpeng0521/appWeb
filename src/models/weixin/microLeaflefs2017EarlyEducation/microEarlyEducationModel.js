...
export default {
    ...
    namespace: 'early_education_namespace',
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microEarlyEducationPage') {
					  _hmt.push(['_trackEvent', '微传单通用早教2017', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let parameter 		= {
				id 			: model.early_education_namespace.activityDataId, 
