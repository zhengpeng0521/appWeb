...
export default {
    ...
    namespace: 'call_phone_second',

    state: {
		playClassName 	        : 'startPlayer',
		swiperSlideClassName	: 'swiper-slide stop-swiping',
		mainData		        : {},
		detailDataSource	    : [],
		nIndex					: 0,
		initWindowHeight		: 0,
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microAutumnRecruitSecondPage') {
					  _hmt.push(['_trackEvent', '微活动2017来电第二版', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let parameter 		= {
				id 			: model.call_phone_second.activityDataId,
