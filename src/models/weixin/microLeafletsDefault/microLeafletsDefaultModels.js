...
export default {
    ...
    namespace: 'microLeafletsDefault',
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microLeafletsDefaultPage') {
					  	_hmt.push(['_trackEvent', '微传单2017默认传单', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      	dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let id 				= initParameter.microLeafletsDefault.activityDataId;
			let name			= initParameter.microLeafletsDefault.babyName;
			let mobile			= initParameter.microLeafletsDefault.babyPhone;
