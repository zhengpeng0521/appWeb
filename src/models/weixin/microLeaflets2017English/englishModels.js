...
export default {
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microEnglishPage') {
					  	_hmt.push(['_trackEvent', '微传单2017英语传单', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      	dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let parameter 	= {
				id 			: model.english2017.activityDataId, 
