...
export default {
    ...
    namespace: 'springOutingActivity',
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microActivitySpringOutingPage') {
					  	_hmt.push(['_trackEvent', '微活动2017春游', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      	dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let id 				= initParameter.springOutingActivity.activityDataId;
			let name			= initParameter.springOutingActivity.babyName;
			let mobile			= initParameter.springOutingActivity.babyPhone;
