...
export default {
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microLeafletsEnglishPage') {
					  	_hmt.push(['_trackEvent', '微传单2016英语传单', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      	dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let id 				= initParameter.englishaActivity.activityDataId;
			let name			= initParameter.englishaActivity.babyName;
			let mobile			= initParameter.englishaActivity.babyPhone;
