...
export default {
    ...
    namespace: 'summer_training_namespace',
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microSummerTrainingPage') {
					  	_hmt.push(['_trackEvent', '微活动2017暑期培训', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      	dispatch({
