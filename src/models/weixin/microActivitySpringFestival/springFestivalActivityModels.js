...
export default {
    ...
    namespace: 'springFestivalActivity',
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microActivitySpringFestivalPage') {
					  	_hmt.push(['_trackEvent', '微活动2017春节', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
					  	dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let id 				= initParameter.springFestivalActivity.activityDataId;
			let name			= initParameter.springFestivalActivity.babyName;
			let mobile			= initParameter.springFestivalActivity.babyPhone;
