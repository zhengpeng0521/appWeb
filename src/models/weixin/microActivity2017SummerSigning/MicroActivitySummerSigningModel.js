...
export default {
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microSummerSigningPage') {
					  _hmt.push(['_trackEvent', '微活动2017暑期报名', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let parameter 		= {
				id 			: model.summer_signing.activityDataId,
