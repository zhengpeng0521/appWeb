...
export default {
    ...
    namespace: 'microActivity2017MayDay',
    ...
    subscriptions: {
	 setup({ dispatch, history }) {
		  history.listen(location => {
			  if (location.pathname === '/microActivity2017MayDayPage') {
				  _hmt.push(['_trackEvent', '微活动2017五一', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
				  dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let id 				= initParameter.microActivity2017MayDay.activityDataId;
			let name			= initParameter.microActivity2017MayDay.babyName;
			let mobile			= initParameter.microActivity2017MayDay.babyPhone;
