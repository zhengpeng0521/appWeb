...
export default {
    ...
    namespace: 'admissionsMicroMarketing',
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microActivityAdmissionsPage') {
					  	_hmt.push(['_trackEvent', '微活动2017招生模板', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      	dispatch({
    ...
    effects: {
		*getAitivityData({payload}, {select, call, put}) {
            ...
            if(ret && ret.errorCode == 9000) {
                ...
                yield put({
                    ...
                    payload: {
                        ...payload,
                        admmMainData	 : JSON.parse(ret.data.activityData.mainData),
                        admmDetailDataSource  : JSON.parse(ret.data.activityData.detailData),
                },
            });
		...
		*submit({payload}, {select, call, put}) {
			...
			let initParameter 	= yield select(state => state);
						console.info(initParameter);

			let id 				= initParameter.admissionsMicroMarketing.activityDataId;
			let name			= initParameter.admissionsMicroMarketing.babyName;
			let mobile			= initParameter.admissionsMicroMarketing.babyPhone;
