...
export default {
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microJuneDayPage') {
					  _hmt.push(['_trackEvent', '微活动2017六一', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      dispatch({
    ...
    effects: {
		...
		*rotateStatus({payload}, {select, call, put}) {
			yield put ({
				type : 'updateState',
				payload : {
					...payload,
					playClassName : payload.playClassName == "startPlayer" ? 'stopPlayer' : 'startPlayer',
				},
			});
		},
		...
		*submit({payload}, {select, call, put}) {
			...
			let parameter 		= {
				id 			: model.microJuneDay.activityDataId, 
