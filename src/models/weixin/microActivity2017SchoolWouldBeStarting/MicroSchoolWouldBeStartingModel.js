...
export default {
    ...
    namespace: 'school_would_be_starting',
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microSchoolWouldBeStartingPage') {
					  _hmt.push(['_trackEvent', '微活动2017开学季模板', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let parameter 		= {
				id 			: model.school_would_be_starting.activityDataId,
