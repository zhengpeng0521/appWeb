...
export default {
    ...
    namespace: 'microActivityDragonBoatFestival',
    ...
    subscriptions: {
         setup({ dispatch, history }) {
              history.listen(location => {
                  if (location.pathname === '/microActivityDragonBoatFestivalPage') {
					  	_hmt.push(['_trackEvent', '微活动2017端午节', `租户ID=${location.query.tenantId || ''}`, `机构ID=${location.query.org_id}`, '-']);
                      	dispatch({
    ...
    effects: {
		...
		*submit({payload}, {select, call, put}) {
			...
			let id 				= initParameter.microActivityDragonBoatFestival.activityDataId;
			let name			= initParameter.microActivityDragonBoatFestival.babyName;
			let mobile			= initParameter.microActivityDragonBoatFestival.babyPhone;
		...
		*increaseShareNum({payload}, {select, call, put}){
			let initParameter 	  = yield select(state => state);
			let id 				  = initParameter.microactivity.activityDataId;
			const {ret, err}      = yield call(increaseShareNumberAPI, id);
		},
