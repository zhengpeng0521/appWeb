...
import styles from './ValidationMobileComponent.less';
...
function MicroVerificationCodeComponent({
	...
	function getVerification() {
		...
		let time = setInterval(function() {
			if (--s == 0) {
				clearInterval(time);
				dp('updateState', {s : undefined, veriText : '点击重新获取', getVeriCodeAction : false})
			} else {
				if(clearTime == false) {
					clearTime = true;
					getVerificationCodeFunction({
	...
	return(
		<div className="js_v_code">
			<WingBlank>
				<div className={styles.js_v_code_title}>请验证您的手机号码</div>
			</WingBlank>
			<div className={styles.js_v_code_content}>
				...
				<div className={styles.js_div}>
					...
					<div className={styles.js_right_vc_phone_div}>
						<InputItem
							{...getFieldProps('phone', {initialValue : defaultMobile})}
							type="phone"
							clear
							placeholder="请输入手机号码"
						>
				...
				<div className={styles.js_submit_btn} onClick={() => submit()}>验证</div>
			</div>
		</div>
    );
}
